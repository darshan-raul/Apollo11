#!/bin/bash
# Verify Set 4: Envoy Gateway + MetalLB (real LoadBalancer IP).
set -e
...
step "Deployments (10 app/infra + 1 envoy + 1 metallb-controller)"
EXPECTED_DEPS=(
...
step "Envoy proxy (auto-created by Gateway, in envoy-gateway-system)"
...
if [[ -n "$SVC" ]]; then
  ...
  if [[ -n "$ENVOY_IP" ]]; then
    pass "MetalLB assigned IP: $ENVOY_IP"
  else
    fail "no LoadBalancer IP assigned to $SVC"
  fi
else
